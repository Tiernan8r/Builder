...
//		if(!(player.hasPermission("build.base"))) {
...
//		}
		...
		if(args.length < 1) {
			player.sendMessage(warning + "You must specify a material for the base.");
			return false;
		}
		...
		cuboid.base(material);
		
		player.sendMessage(good + "A base for the Cuboid between " + highlight + location1.getX() + " " + location1.getY() + " " + location1.getZ() + good + " and " + highlight + location2.getX() + " " + location2.getY() + " " + location2.getZ() + good + " has been built from the Material " + highlight + materialName + good + ".");
